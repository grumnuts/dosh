...
export function InvestmentsReport() {
  ...
  const visibleMonths = new Set(investmentTimeline.windowedMonths)
  const displayChartData = historyData?.chartData.filter((row) => visibleMonths.has(String(row.month))).map((row) => {
    if (selectedTicker) {
