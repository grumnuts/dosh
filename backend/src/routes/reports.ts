...
export async function reportRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/reports/invsout', { preHandler: authenticate }, async (request, reply) => {
    ...
    // Include all transactions (regular, transfer, cover, sweep) except system-category
    // entries (starting balance, reconciliation) which are accounting artefacts, not real flows
    const accountFilter = accountId ? 'AND t.account_id = ?' : ''
    const params = accountId ? [year, accountId] : [year]
      ...
      `SELECT strftime('%m', t.date) AS month,
       ...
       WHERE strftime('%Y', t.date) = ?
         AND (
           ...
           OR t.category_id NOT IN (SELECT id FROM budget_categories WHERE is_system = 1)
         )
         ${accountFilter}
       GROUP BY month
       ORDER BY month`,
